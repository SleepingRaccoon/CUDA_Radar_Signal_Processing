...
与 pc_mtd_v1_cupy.py 逐条对应：九个步骤，每步一个 kernel 或一次 cufft 调用
输入：output/py/pc_mtd_v1_rxbb.npy（pc_mtd_v1.py 生成的仿真回波基带，[Nc][Nw] complex64）
输出：output/cpp/pc_mtd_v1_rdm.npy（距离-多普勒图 [Nr][Nd] complex64）
布局：rd 布局 [Nr][Nd]，与 CPU 版一致
计时：REPS 次取平均，用 cudaEvent；IO = 读 npy + 主存到显存 + 显存回主存 + 写 npy
      回波生成不计时；LFM 频谱是雷达系统常驻量，预先算好，不计时
参数与 pc_mtd_v1.py 严格一致，改一处必须三边一起改

Build: nvcc -O2 -std=c++17 -Xcompiler /utf-8
       -o src/detect_and_track/Range_and_Velocity/code/cpp/pc_mtd_v1.exe
       src/detect_and_track/Range_and_Velocity/code/cpp/pc_mtd_v1.cu -lcufft
Run:   src/detect_and_track/Range_and_Velocity/code/cpp/pc_mtd_v1.exe
*/
...
static const int    N_fft = 2048;                                  // 距离维 FFT 点数
...
// 步骤 1、7：把 [rows][src_cols] 拷到 [rows][dst_cols]，dst 需先清零
...
// 步骤 5：乘 1/N_fft 并截取前 dst_cols 列
...
// 步骤 6：加窗 + 转置，[Nc][Nr] -> [Nr][Nc]，dst[j*Nc + i] = pc[i*Nr + j] * w[i]
...
// 步骤 9：每行循环右移 cols/2，等价于 np.fft.fftshift(axis=1)
...
static void make_hamming_like_hann(float *w, int M)
{
    // 周期汉宁窗，与 np.hanning(M+1)[:-1] 一致
    for (int n = 0; n < M; ++n) {
        w[n] = (float)(0.5 - 0.5 * cos(2.0 * PI * n / M));
    }
}
...
int main(void)
{
    char p_rxbb[512], p_rdm_ref[512], p_timing[512], p_out[512];
    path_near_src(p_rxbb, sizeof(p_rxbb), "../../output/py/pc_mtd_v1_rxbb.npy");
    path_near_src(p_rdm_ref, sizeof(p_rdm_ref), "../../output/py/pc_mtd_v1_rdm.npy");
    path_near_src(p_timing, sizeof(p_timing), "../../output/py/pc_mtd_v1_timing.json");
    path_near_src(p_out, sizeof(p_out), "../../output/cpp/pc_mtd_v1_rdm.npy");
    ...
    // ---- 主机侧输入 ----
    void *h_rxbb = NULL, *h_rdm_ref = NULL;
    long a0 = 0, a1 = 0, b0 = 0, b1 = 0, off_rxbb = 0, off_ref = 0;
    load_npy(p_rxbb, &h_rxbb, &a0, &a1, &off_rxbb);
    load_npy(p_rdm_ref, &h_rdm_ref, &b0, &b1, &off_ref);
    if (a0 != Nc || a1 != Nw) { printf("rxbb shape mismatch: %ld x %ld\n", a0, a1); exit(1); }
    if (b0 != Nr || b1 != Nd) { printf("rdm shape mismatch: %ld x %ld\n", b0, b1); exit(1); }

    float2 *h_rdm = (float2 *)malloc((size_t)Nr * Nd * 8);     // device 回传结果
    float *h_W = (float *)malloc((size_t)Nc * sizeof(float));
    float2 *h_stx = (float2 *)malloc((size_t)N_fft * 8);
    make_hamming_like_hann(h_W, Nc);
    make_s_tx_bb(h_stx, Ns);
    for (int i = Ns; i < N_fft; ++i) h_stx[i] = make_float2(0.0f, 0.0f);

    // ---- 设备侧缓冲 ----
    float2 *d_rxbb, *d_pad, *d_spec, *d_pc_full, *d_pc, *d_win, *d_win_pad, *d_rdm, *d_shift, *d_H, *d_stx;
    float *d_W;
    CHECK_CUDA(cudaMalloc(&d_rxbb, (size_t)Nc * Nw * 8));
    ...
    CHECK_CUDA(cudaMalloc(&d_rdm, (size_t)Nr * Nd * 8));
    ...
    // ---- 常驻量准备（不计时）：H = conj(FFT(s_tx_bb))，与 cuda 版的准备阶段一致 ----
    cufftHandle plan_h, plan_range, plan_dopp;
    CHECK_CUFFT(cufftPlan1d(&plan_h, N_fft, CUFFT_C2C, 1));
    CHECK_CUFFT(cufftPlan1d(&plan_range, N_fft, CUFFT_C2C, Nc));
    CHECK_CUFFT(cufftPlan1d(&plan_dopp, Nd, CUFFT_C2C, Nr));

    CHECK_CUDA(cudaMemcpy(d_stx, h_stx, (size_t)N_fft * 8, cudaMemcpyHostToDevice));
    CHECK_CUFFT(cufftExecC2C(plan_h, (cufftComplex *)d_stx, (cufftComplex *)d_H, CUFFT_FORWARD));
    conj_kernel<<<(N_fft + 255) / 256, 256>>>(d_H, N_fft);
    CHECK_CUDA(cudaDeviceSynchronize());

    // ---- 九个步骤：与 pc_mtd_v1_cupy.py 的 run_once 逐条对应 ----
    CHECK_CUDA(cudaMemcpy(d_rxbb, h_rxbb, (size_t)Nc * Nw * 8, cudaMemcpyHostToDevice));
    CHECK_CUDA(cudaDeviceSynchronize());

    // 第一次执行：这一次的结果就是输出
    CHECK_CUDA(cudaMemset(d_pad, 0, (size_t)Nc * N_fft * 8));
    pad_kernel<<<(Nc * Nw + 255) / 256, 256>>>(d_rxbb, d_pad, Nc, Nw, N_fft);
    CHECK_CUFFT(cufftExecC2C(plan_range, (cufftComplex *)d_pad, (cufftComplex *)d_spec, CUFFT_FORWARD));
    mul_H_kernel<<<(Nc * N_fft + 255) / 256, 256>>>(d_spec, d_H, N_fft, (long)Nc * N_fft);
    CHECK_CUFFT(cufftExecC2C(plan_range, (cufftComplex *)d_spec, (cufftComplex *)d_pc_full, CUFFT_INVERSE));
    ...
    CHECK_CUFFT(cufftExecC2C(plan_dopp, (cufftComplex *)d_win_pad, (cufftComplex *)d_rdm, CUFFT_FORWARD));
    fftshift_kernel<<<(Nr * Nd + 255) / 256, 256>>>(d_rdm, d_shift, Nr, Nd);
    CHECK_CUDA(cudaDeviceSynchronize());

    CHECK_CUDA(cudaMemcpy(h_rdm, d_shift, (size_t)Nr * Nd * 8, cudaMemcpyDeviceToHost));
    save_npy(p_out, h_rdm, Nr, Nd);

    // 自校验：峰值位置
    const double tgt[2][2] = {{2000.0, 40.0}, {4000.0, -50.0}};
    double range_cell = c / (2.0 * fs);
    double v_cell = lam * fr / (2.0 * Nd);
    double r0_axis = c / 2.0 * Tp;
    printf("self-check  (peak vs ground truth)\n");
    for (int k = 0; k < 2; ++k) {
        int lo = 0, hi = Nr - 1;
        for (int j = 0; j < Nr; ++j) {
            if (r0_axis + j * range_cell >= tgt[k][0] - 5.0 * range_cell) { lo = j; break; }
        }
        for (int j = Nr - 1; j >= 0; --j) {
            if (r0_axis + j * range_cell <= tgt[k][0] + 5.0 * range_cell) { hi = j; break; }
        }
        double best = -1.0;
        int best_i = 0, best_j = lo;
        for (int i = 0; i < Nd; ++i) {
            for (int j = lo; j <= hi; ++j) {
                float2 v = h_rdm[(size_t)j * Nd + i];
                double a = (double)v.x * v.x + (double)v.y * v.y;
                if (a > best) { best = a; best_i = i; best_j = j; }
            }
        }
        double vv = -(best_i - Nd / 2) * v_cell;
        printf("  target %d   true R = %8.1f m  v = %+7.2f m/s   measured R = %8.1f m  v = %+7.2f m/s\n",
               k + 1, tgt[k][0], tgt[k][1], r0_axis + best_j * range_cell, vv);
    }

    // 与 CPU 结果的误差
    float *pr = (float *)h_rdm;
    float *pg = (float *)h_rdm_ref;
    double err = 0.0, ref = 0.0;
    for (size_t i = 0; i < (size_t)Nr * Nd; ++i) {
        double dx = (double)pr[2 * i] - (double)pg[2 * i];
        double dy = (double)pr[2 * i + 1] - (double)pg[2 * i + 1];
        double dd = sqrt(dx * dx + dy * dy);      // 与 python 侧一致，按复数模比
        if (dd > err) err = dd;
        double ax = (double)pg[2 * i], ay = (double)pg[2 * i + 1];
        double aa = sqrt(ax * ax + ay * ay);
        if (aa > ref) ref = aa;
    }
    printf("error on rdm [Nr][Nd]  (CUDA vs CPU)\n");
    printf("  %-34s %.6e\n", "max absolute error", err);
    printf("  %-34s %.6e\n", "max relative error", err / ref);

    // ---- 计时：REPS 次取平均，每段用 cudaEvent ----
    cudaEvent_t ev[4];
    ...
    for (int rep = 0; rep < REPS; ++rep) {
        ...
        {
            FILE *f = fopen(p_rxbb, "rb");
            if (f == NULL) { printf("cannot open %s\n", p_rxbb); exit(1); }
            fseek(f, off_rxbb, SEEK_SET);
            if (fread(h_rxbb, 8, (size_t)Nc * Nw, f) != (size_t)Nc * Nw) { printf("short read\n"); exit(1); }
            fclose(f);
            CHECK_CUDA(cudaMemcpy(d_rxbb, h_rxbb, (size_t)Nc * Nw * 8, cudaMemcpyHostToDevice));
        }
    ...
    printf("  %-34s %10.3f ms\n", "compute (9 steps, kernel + cufft)", t_calc);
    ...
    if (txt != NULL) {
        ...
        printf("speedup vs CPU  (pc_mtd_v1_timing.json)\n");
        ...
        printf("speedup vs CPU  (missing pc_mtd_v1_timing.json)\n");
    }

    printf("saved: %s\n", p_out);

    free(h_rxbb); free(h_rdm_ref); free(h_rdm); free(h_W); free(h_stx);
    return 0;
}
